...
4. **实现工具**
   - Puppeteer（Node.js）或 Selenium（Python）
   - 截图功能记录关键时刻
   - 日志输出游戏进度
...
|------|------|---------|---------|-------------|---------|---------|------|
| 🥇 1 | simple_fast.json | 7,356 | 9,368 | 620 (2^9.3) | 1024 | 500 | 简单快速模型 |
| 🥈 2 | high_performance.json | 7,165 | 9,364 | 543 (2^9.1) | 1024 | 1000 | 高性能模型 |
| 🥉 3 | balanced_best.json | 2,594 | 7,116 | 204 (2^7.7) | 512 | 5000 | 平衡模型（包含平滑度） |
...
1. **无移动限制时的真实性能**（max_moves = 10000）
   - 100 节点: 15,508 平均分（24.0 games/sec）
   - 500 节点: 19,708 平均分（2.3 games/sec）
   - 1000 节点: 25,421 平均分（0.8 games/sec）
   - 2000 节点: 24,502 平均分（0.2 games/sec）
   - 所有配置都能达到 2048 瓦片

2. **性能差异分析**
   - 之前报告的 7,356 分可能是由于人为的移动限制
   - 无限制时，simple_fast 性能提升 ~2.7 倍
   - 1000 节点是性能最优点，之后收益递减

3. **速度与质量权衡**
   - 100 节点比 1000 节点快 24 倍
   - 1000 节点比 100 节点得分高 64%
   - 最佳平衡点：500-1000 节点
